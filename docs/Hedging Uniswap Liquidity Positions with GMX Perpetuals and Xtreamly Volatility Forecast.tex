...
We propose a hedging mechanism applicable to Uniswap V4 \& V3 \textit{liquidity provider} (LP) positions with GMX perpetual futures to reduce risk and enhance performance. This section outlines key components of the strategy: the Uniswap LP position, the GMX perpetual futures contract, and Xtreamly’s volatility forecasting component.
...
Takeaways:
\begin{itemize}
	\item \textbf{\% Returns LP Positions}: In most of pools positions had on average negative return performance due to experienced IL.
	\item \textbf{\% Improvement with Hedging}: In almost all pools we identified average positive improvement with hedging, which adds up in total of 1,313k USD in extra profit. 
	\item \textbf{\# Perpetuals}: On average each LP position was hedged 3-4 times, stating dynamic nature of hedging within LP position lifetime.
\end{itemize}
