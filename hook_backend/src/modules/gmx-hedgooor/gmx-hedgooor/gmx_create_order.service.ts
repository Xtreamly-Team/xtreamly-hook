...
const logger = new Logger('GmxOrderService');
...
export async function shortIt() {
  logger.log('Starting to create short position...');
  ...
  try {
    logger.log(`Attempting to create short order for address: ${account.address}...`);
  ...
  //  console.log("Available markets:", marketsInfoData ? Object.keys(marketsInfoData) : []);
   ...
   logger.debug(`Market name: ${marketInfo.name}`);
   logger.debug(`Index token: ${marketInfo.indexToken.symbol}`);
  ...
  //////////////////////////////////////////////////////////////////
    ...
    for (let index = 0; index < positionsArray.length; index++) {
      ...
      logger.debug(index, formatPosition(position));
    }


  logger.debug(positionsArray)
    //////////////////////////////////////////////////////////////////
    ...
    try {
      ...
      logger.log("Increase order created successfully");

      // Store the position via API
      await storeHedgePositionViaAPI(
        account.address,
        Number(formatUnits(sizeDeltaUsd, 30)),
        result?.txHash,
        marketInfo.name,
      ...
      logger.error(`Failed to create GMX order: ${orderError.message || orderError}`);
      logger.error("Failed to create increase order:", orderError);
      throw new Error(`Order creation failed: ${orderError.message || orderError}`);
    }
  } catch (error) {
    logger.error(`Failed to create short: ${error.message || error}`);
    logger.error("Failed to create increase order:", error);
    ...
    //////////////////////////////////////////////////////////////////
    // Close short order with order.position[0]
    //////////////////////////////////////////////////////////////////
export async function closeShort() {
  logger.log('Starting to close short position...');
  
  try {
    logger.log(`Attempting to close short position for address: ${account.address}`);
    
    // Get market info and tokens data
    ...
    try {
      ...
      // Store the closed position via API
      await storeClosedPositionViaAPI(
        account.address,
        position,
        result?.txHash,
        marketInfo,
...
/**
 * Store a hedge position via API call to the position controller
 * This avoids direct repository interaction and uses the existing endpoint
 */
async function storeHedgePositionViaAPI(
  ...
  try {
    // Always use the fallback address that we know exists in the database
    const fallbackWalletAddress = '0xF6581727750412981985e8744916DCd95F13Abe5';
    
    // Create a position with only necessary data
    const positionData = {
      walletAddress: fallbackWalletAddress, // Always use fallback address
      tokenA: indexToken,     // Use the index token as tokenA
      tokenB: collateralToken, // Use the collateral token as tokenB
      amountA: 0,             // Placeholder
      amountB: 0,             // Placeholder
      lowerTick: 0,           // Placeholder
      upperTick: 0,           // Placeholder
      hedgeAmount: sizeInUsd, // Store size in USD as hedge amount
      status: 'active',       // Using a valid status from the enum options
      uniswapPositionId: null,
      gmxPositionId: txHash || 'pending',
      metadata: {
        gmx: {
          marketName,
          indexToken,
          collateralToken,
          sizeDeltaUsd: sizeInUsd.toString(),
          txHash,
          timestamp: new Date().toISOString(),
          originalWalletAddress: walletAddress, // Store the original wallet address in metadata
          isHedge: true // Flag to indicate this is a hedge position
        }
      }
    };
    ...
    const headers = {
      'x-api-key': 'panos',
      'user': fallbackWalletAddress // Use the fallback address in the headers too
    };
    
    logger.log(`Sending API request to ${apiBaseUrl}/position/create-LP-position with headers:`, headers);
    logger.log(`Using fallback wallet address: ${fallbackWalletAddress} (original wallet: ${walletAddress})`);
    
    // Call the existing create position endpoint with auth headers
    const response = await axios.post(
      `${apiBaseUrl}/position/create-LP-position`,
      positionData,
      {
        headers: headers
      }
    ...
    logger.log(`Hedge position stored in database with ID: ${response.data.id}`);
    return response.data;
  } catch (error) {
    logger.error(`Failed to store hedge position via API: ${error.message}`);
    if (error.response) {
...
/**
 * Store a closed hedge position via API call to the position controller
 * This avoids direct repository interaction and uses the existing endpoint
 */
async function storeClosedPositionViaAPI(
  walletAddress: string,
  position: any,
  txHash: string | undefined,
  marketInfo: any,
  ...
  try {
    ...
    const positionData = {
      ...
      lowerTick: 0,           // Placeholder
      upperTick: 0,           // Placeholder
      hedgeAmount: Number(formatUnits(position.sizeInUsd, 30)), // Store size in USD as hedge amount
      status: 'closed',       // Using string rather than enum for API compatibility
      uniswapPositionId: null,
      gmxPositionId: txHash || `close-${position.key}`,
      metadata: {
        gmx: {
          marketName: marketInfo.name,
          indexToken: marketInfo.indexToken.symbol,
          collateralToken: collateralToken.symbol,
          sizeDeltaUsd: formatUnits(position.sizeInUsd, 30),
          collateralAmount: formatUnits(position.collateralAmount, collateralToken.decimals),
          pnl: formatUnits(position.pnl || 0n, 30),
          txHash,
          timestamp: new Date().toISOString(),
          isClose: true,
          originalPositionKey: position.key,
          originalWalletAddress: walletAddress // Store the original wallet address in metadata
        }
      }
    };
    ...
    logger.log(`Closed position stored in database with ID: ${response.data.id}`);
    return response.data;
  } catch (error) {
    logger.error(`Failed to store closed position via API: ${error.message}`);
    if (error.response) {
      logger.error(`Response status: ${error.response.status}`);
      logger.error(`Response data:`, error.response.data);
    }
